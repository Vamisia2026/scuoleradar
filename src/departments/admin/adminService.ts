...
/**
 * Chiama la Edge `admin` (azione + payload) e normalizza la risposta.
 * Esportata per i servizi del dipartimento (es. `automazioniService.ts`).
 */
export async function chiamaAdmin<T>(action: string, payload?: Record<string, unknown>): Promise<T> {
