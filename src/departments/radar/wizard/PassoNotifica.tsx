...
export function PassoNotifica({ notifica, piano, rapida }: PassoNotificaProps) {
  ...
  return (
            <div className="animate-fade-in">
              ...
              <div className="mt-4 flex items-start gap-2.5 rounded-xl border border-accent-200 bg-accent-50 px-4 py-3 text-sm text-accent-800">
                <Zap className="mt-0.5 h-4 w-4 shrink-0 text-accent-500" />
                <p className="leading-relaxed">
                  <strong>Telegram = avvisi ISTANTANEI.</strong> Via email ricevi{' '}
                  <strong>solo i riepiloghi</strong>: per non perdere le opportunità che escono e
                  scadono in poche ore, collega Telegram adesso. Bastano 10 secondi e puoi
                  scollegarlo quando vuoi.
                </p>
              </div>
                ...
                {ospite && rapida && (
                  <div className="rounded-xl border border-primary-200 bg-white p-4">
                    ...
                    <p className="mt-1 text-xs leading-relaxed text-primary-500">
                      ...
                      provincia che hai già indicato vengono salvati nel tuo profilo.
                    </p>
                    <button
                      type="button"
                      onClick={rapida.onGoogle}
                      disabled={rapida.googleInCorso}
                      className="mt-3 inline-flex w-full items-center justify-center gap-2.5 rounded-xl border-2 border-primary-500 bg-primary-50 px-4 py-3 text-sm font-bold text-primary-800 shadow-soft transition hover:bg-primary-100 disabled:cursor-wait disabled:opacity-70"
                    >
