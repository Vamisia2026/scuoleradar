...
import { chiaveInterpelloDaParam, eUuid, urlSchedaInterpello } from '../src/lib/interpelloRouting.ts';
...
check('etichetta scheda interna', "Apri la scheda dell'avviso", etichettaFonteLink('https://www.scuoleradar.it/interpello/abc'));
check(
...
console.log('\n— Deep link `/interpello/:id` (niente rimbalzo sulla Home) —');
const fallback = linkOpportunita({ ...interpello, link: null }, 'https://scuoleradar.it/dashboard/radar');
check('fallback → scheda interna', 'https://scuoleradar.it/interpello/abc123hash', fallback);
check(
  'id uuid → colonna id',
...
check('urlSchedaInterpello (slash normalizzato)', 'https://www.scuoleradar.it/interpello/x1', urlSchedaInterpello('https://www.scuoleradar.it/', 'x1'));

const app = readFileSync('src/App.tsx', 'utf8');
check('rotta /interpello/:id registrata', true, /path="\/interpello\/:id"/.test(app));
check('deep link email punta a /interpello/', true, fallback.includes('/interpello/'));
